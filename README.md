...
**Windows 上的应用级 `Alt+Tab`,外观来自 PowerToys Window Hopper。**

两个快捷键,两种职责——和 macOS 完全一致:

| 快捷键 | 在什么之间切换 | 行为 |
|---|---|---|
| `Alt+Tab` | **应用** | 每个应用只占一个条目,按最近使用排序;前台应用的兄弟窗口绝不出现,所以 `Alt+Tab` 永远落在*另一个*应用上 |
...
原生 Windows `Alt+Tab` 把所有顶层窗口平铺列出,既无法按应用分组,也没有任何受支持的方法去过滤原生列表(隐藏/cloak/改样式等各种技巧都会泄漏到任务栏或虚拟桌面)。现有的切换器要么换成一个没有预览的纯图标条,要么不按应用分组,要么不能在 Windows 10 上运行。

干净的解法是拥有整个浮层:接管 `Alt+Tab` 按键,自己分组窗口,用 DWM 缩略图渲染预览。移植 Window Hopper 的 UI 则让 `Alt+Tab` 与 `Alt+\`` 这两个半边工作流在视觉上保持一致。

## 构建

无需安装任何工具链——Windows 自带 C# 编译器:
